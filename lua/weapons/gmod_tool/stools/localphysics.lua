...
function TOOL.BuildCPanel(cp)
	...
	cp:AddControl("Checkbox",{Label = "Hit Surface Defines Floor", Description = "If checked, the surface shot by the tool will determine which way 'up' is. Otherwise it counts up as up, meaning your camera will be straight on a diagonal surface.",
		...
		cp:AddControl("Checkbox",{Label = "Apply to entire contraption", Description = "If checked, the hull will be applied to the entire contraption. Otherwise, it will only apply to the entity you click.",
							  Command = "localphysics_contraption"})
...
function TOOL:LeftClick(tr)
	local ent = tr.Entity
	local contraption = self:GetClientNumber("contraption")
	...
	if self:GetClientNumber("gravnormal") > 0 then
		GravHull.UpdateHull(ent,contraption,tr.HitNormal)
	else
